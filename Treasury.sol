...
contract TimeTreasury is Ownable {
    ...
    ITIMEERC20 public immutable AMZN;
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint AMZN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        limitRequirements(msg.sender, send_);
        AMZN.mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn AMZN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        AMZN.burnFrom( msg.sender, value );
    ...
    /**
        @notice allow approved address to repay borrowed reserves with AMZN
        @param _amount uint
     */
    function repayDebtWithTime( uint _amount ) external {
        ...
        AMZN.burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, address(AMZN), _amount, _amount );
    }
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( AMZN.totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns AMZN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match AMZN decimals
            value_ = _amount.mul( 10 ** AMZN.decimals() ).div( 10 ** IERC20( _token ).decimals() );
