...
contract TimeBondDepository is Ownable {
    ...
    IERC20 public immutable Amazon; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Amazon when receives principle
    ...
    struct Bond {
        uint payout; // Amazon remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Amazon ( underflow protection )
        ...
        uint balanceBefore = Amazon.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Amazon.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Amazon.balanceOf(address(this)), "Not enough Time to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Amazon.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Amazon.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Amazon.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Amazon.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Amazon.totalSupply();
    ...
    /**
     *  @notice calculate amount of Amazon available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Amazon) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Amazon, "NAT" );
