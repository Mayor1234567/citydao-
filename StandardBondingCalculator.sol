...
contract AmazonBondingCalculator is IBondingCalculator {

    using FixedPoint for *;
    using LowGasSafeMath for uint;
    using LowGasSafeMath for uint112;

    IERC20 public immutable AMZN;

    constructor( address _AMZN ) {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(AMZN) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(AMZN), "not a Time lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** AMZN.decimals() ) ).div( getTotalValue( _pair ) );
    }
}
