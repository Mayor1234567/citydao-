...
contract TimeStaking is Ownable {
    ...
    IERC20 public immutable Amazon;
    IMemo public immutable sAmazon;
    ...
    constructor ( 
        ...
        require( _Amazon != address(0) );
        Amazon = IERC20(_Amazon);
        require( _sAmazon != address(0) );
        sAmazon = IMemo(_sAmazon);
    ...
    /**
        @notice stake Amazon to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Amazon.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( sAmazon.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        sAmazon.safeTransfer( address(warmupContract), _amount );
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = sAmazon.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit sAmazon in warmup and retrieve Amazon
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = sAmazon.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Amazon.safeTransfer( msg.sender, info.deposit);
    ...
    /**
        @notice redeem sAmazon for Amazon
        @param _amount uint
        @param _trigger bool
     */
    function unstake( uint _amount, bool _trigger ) external {
        ...
        sAmazon.safeTransferFrom( msg.sender, address(this), _amount );
        Amazon.safeTransfer( msg.sender, _amount );
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            sAmazon.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = sAmazon.circulatingSupply();
    ...
    /**
        @notice returns contract Amazon holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Amazon.balanceOf( address(this) ).add( totalBonus );
    }
